...
def main():
    ...
    session_id = "11717272380955906268"
    
    # Read specification lines from the backlog file
    backlog_path = Path("elwas_toolkit/Aquarevier_Map_Backlog.md")
    lines = backlog_path.read_text(encoding="utf-8").splitlines()
    
    # Extract lines 4167 to 4308 (0-indexed: lines[4166:4308])
    spec_lines = lines[4166:4308]
    spec_text = "\n".join(spec_lines)
    
    prompt = (
        "Here are the detailed specification requirements for the task "
        "\"Ökologische Durchgängigkeit an Querbauwerken (Fischwanderung)\" "
        "as requested:\n\n"
        f"{spec_text}\n\n"
