...
try:
    ...
    # Let's find all CSS files
    css_files = re.findall(r'href=["\']([^"\']+\.css)["\']', html)
    print("CSS files:")
    for css in css_files:
        print("-", css)
        
    # Search for all images or inline svgs or background-image styles in HTML
    bg_imgs = re.findall(r'background-image:\s*url\(["\']?([^"\')]+)["\']?\)', html)
    print("Background images in HTML:")
    for bg in bg_imgs:
        print("-", bg)
        
    # Search the CSS files for images containing 'logo' or 'header'
    for css in css_files:
        ...
        try:
            ...
            with urllib.request.urlopen(req_css, timeout=3) as res_css:
                ...
                logo_urls = [u for u in urls_in_css if 'logo' in u.lower() or 'fiw' in u.lower()]
                if logo_urls:
