...
def spawn_session(prompt: str, api_key: str) -> str:
    ...
    payload = json.dumps({
        "prompt": prompt,
        "sourceContext": {
            "source": "sources/github/Dtunder/adb_aquarevier_map",
            "githubRepoContext": {
                "startingBranch": "main"
            }
        },
...
def get_session_status(session_id: str) -> str:
    ...
    """
    rc, stdout, stderr = run_cmd("C:\\Users\\user\\sonu-cli-advanced\\bin\\jules.exe remote list --session")
    if rc != 0:
...
def pull_and_apply(session_id: str) -> bool:
    ...
    cmd = f"C:\\Users\\user\\sonu-cli-advanced\\bin\\jules.exe remote pull --session {session_id} --apply"
