...
for i in range(pbar_css_start, len(index_lines)):
    if '/* Responsive adjustments for presentation bar */' in index_lines[i]:
        # we need to find the end of the media query
        pass
    if '        .main-partner-label {' in index_lines[i]:
        pbar_css_end = i
        break
...
for i in range(pbar_js_start, len(index_lines)):
    if '            // Check URL for ?lat=x&lng=y&zoom=z' in index_lines[i]:
        pbar_js_end = i
        break
...
insert_js = -1
for i, line in enumerate(internal_lines):
    if '            // Check URL for ?lat=x&lng=y&zoom=z' in line:
        insert_js = i
        break
