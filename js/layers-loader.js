...
function addGeoLayer(cfg, map, overlayMaps, layerDataStore) {
  ...
  function buildPopupHtml(p) {
    ...
    for (const field of (cfg.popupFields || [])) {
      ...
      // value is already safe if it came from `p`
      // For backwards compatibility and dynamically evaluated expressions, we keep escapeHtml where we construct strings
      // first field (📍) has no label prefix, just the value
      if (field.label === '📍') {
        html += `<div class="popup-detail">📍 ${value}</div>`;
      } else {
        html += `<div class="popup-detail">${escapeHtml(tLabel(field.label))}${glossarSpan(field.glossar)}: ${value}${escapeHtml(field.suffix || '')}</div>`;
      }
    }
