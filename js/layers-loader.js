...
function addGeoLayer(cfg, map, overlayMaps, layerDataStore) {
  ...
  function buildPopupHtml(p) {
    ...
    // Create a sanitized copy of all properties
    // We only escape strings. Numbers, arrays, objects, etc. are passed as-is.
    const sanitizedP = {};
    for (const key in p) {
    ...
    for (const field of (cfg.popupFields || [])) {
      let value;
      if (field.expr) {
        // Expressions should receive the sanitized properties so any injection points they use are safe.
        // If an expression builds HTML (e.g. <a> tag), it will build it with sanitized data.
        // We do NOT escape the output of expr() so the HTML tags it generates remain valid.
        value = field.expr(sanitizedP);
      } else {
        // Direct field access uses the already sanitized value.
        value = sanitizedP[field.field];
      }
      if (!value) continue;

      // We do not double-escape here because the source data (sanitizedP) is already safe.
      const safeVal = value;
