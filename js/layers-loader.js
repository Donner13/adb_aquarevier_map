  ...
  function buildPopupHtml(p) {
    ...
    for (const field of (cfg.popupFields || [])) {
      ...
      if (typeof value === 'object') {
         safeVal = escapeHtml(JSON.stringify(value));
      } else {
         safeVal = escapeHtml(String(value));
      }
    ...
    if (cfg.pegelStats && p.mq_m3s !== undefined && p.mq_m3s !== null && p.mq_m3s !== '') {
      const getVal = (val) => (val !== undefined && val !== null && val !== '') ? escapeHtml(String(val)) : '–';
      html += `<div class="popup-detail">📊 NQ<span class="glossar-icon" data-glossar="NQ">i</span>: ${getVal(p.nq_m3s)}, MNQ<span class="glossar-icon" data-glossar="MNQ">i</span>: ${getVal(p.mnq_m3s)}, MQ<span class="glossar-icon" data-glossar="MQ">i</span>: ${getVal(p.mq_m3s)}, HQ<span class="glossar-icon" data-glossar="HQ">i</span>: ${getVal(p.hq_m3s)} m³/s</div>`;
    ...
    if (cfg.pegelStats && p.mq_m3s !== undefined && p.mq_m3s !== null && p.mq_m3s !== '' && p.upstream_data_available && p.upstream_mq_pct !== null && p.upstream_mq_pct !== undefined) {
      const pctStr = Number(p.upstream_mq_pct).toFixed(2).replace('.', ',');
      const betriebeHinweis = p.upstream_betriebe_mit_wert > 0
        ? ` (${escapeHtml(String(p.upstream_betriebe_mit_wert))} Betrieb(e) mit Mengenangabe oberhalb)`
