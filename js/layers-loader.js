...
function addGeoLayer(cfg, map, overlayMaps, layerDataStore) {
  ...
  function buildPopupHtml(p) {
    ...
    for (const field of (cfg.popupFields || [])) {
      let value;
      if (field.expr) {
        // Evaluate the expression with original p (which is required so expressions don't receive double-escaped or corrupted data).
        // Since expressions can safely generate HTML intentionally (e.g. eBird <a> tag), we must NOT escape their return value.
        // We trust the layer-config expressions to sanitize specific properties if they need to, but the properties inside `sanitizedP` are safe.
        // To enforce XSS protection on expressions without breaking HTML, the expression MUST operate on `sanitizedP` if it injects data.
        // BUT expressions sometimes need unescaped data for URLs.
        // We will execute the expression with `sanitizedP` so that injected data is safe, but we will NOT escape the result.
        // Wait, if an expression builds a URL like `<a href="${p.url}">`, and `p.url` is escaped, it's fine.
        value = field.expr(sanitizedP);
      } else {
        value = sanitizedP[field.field];
      }
      if (!value) continue;

      // The value is either already sanitized because it came directly from `sanitizedP`,
      // or it is the raw HTML generated by an expression that operated on safely sanitized properties (`sanitizedP`).
      // Therefore, no further escaping is needed here, preventing the aggressive double-escaping of intentional HTML tags.
      const safeVal = value;
