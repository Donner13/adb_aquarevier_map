...
function addGeoLayer(cfg, map, overlayMaps, layerDataStore) {
  ...
  function buildPopupHtml(p) {
    // Original properties safe copy without escaping (to prevent double-escaping)
    const rawP = p || {};

    // XSS-Sanitisierung fuer alle String-Eigenschaften (rekursiv)
    ...
    for (const field of (cfg.popupFields || [])) {
      let value;
      if (field.expr) {
        // field.expr expects escaped values to build HTML safely, but some expressions (like eBird) return HTML tags.
        // We pass the ALREADY escaped `p` into expr, so it operates on safe data.
        // We do NOT double escape the return value of expr so HTML like <a> tags are preserved.
        value = field.expr(p);
      } else {
        // If it's a direct field access, we grab the ALREADY escaped value from `p`.
        value = p[field.field];
      }
      if (!value) continue;

      // Since value is either already safely escaped from `p` or is safe HTML generated by `expr(p)`,
      // we can use it directly without another layer of `escapeHtml(value)`.
      const safeVal = value;
