...
(function() {
    ...
    window.renderBookmarksList = function() {
        ...
        if (bookmarks.length === 0) {
            container.innerHTML = `
                <div style="font-size: 0.65625rem; color: #64748b; text-align: center; padding: 6px;">
        ...
        bookmarks.forEach(bm => {
            html += `
                <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 4px; padding: 6px 8px; margin-bottom: 4px; display: flex; justify-content: space-between; align-items: center; font-size: 0.6875rem;">
                    <div style="cursor: pointer; flex: 1;" onclick="applyBookmark('${bm.id}')">
                        ...
                        <div style="font-size: 0.59375rem; color: #64748b;">Zoom ${bm.zoom} • ${bm.date}</div>
                    </div>
                    <button type="button" aria-label="Löschen" title="Löschen" style="background: transparent; border: none; color: #ef4444; font-size: 0.875rem; cursor: pointer;" onclick="deleteBookmark('${bm.id}')">✕</button>
                </div>
            `;
        });
