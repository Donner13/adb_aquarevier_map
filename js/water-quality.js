...
(function() {
    ...
    window.openWrrlQualityModal = async function(riverName) {
        ...
        if (!data) {
            modal.innerHTML = `
                <div style="background: #ffffff; width: 100%; max-width: 440px; border-radius: 12px; padding: 20px; text-align: center;">
                    ...
                    <div style="font-size: 0.6875rem; color: #64748b; margin-bottom: 14px;">Die Messwerte für '${escapeHtml(name)}' konnten vom Server nicht geladen werden.</div>
        ...
        modal.innerHTML = `
                <div style="background: #ffffff; width: 100%; max-width: 480px; border-radius: 12px; box-shadow: 0 20px 40px rgba(0,0,0,0.3); overflow: hidden; font-size: 0.75rem;">
                <div style="background: #0284c7; color: #ffffff; padding: 14px 18px; display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <div style="font-size: 0.9375rem; font-weight: 700;">🐟 WRRL-Gewässergüte: ${escapeHtml(name)}</div>
                        <div style="font-size: 0.65625rem; opacity: 0.9;">Wasserrahmenrichtlinie (WRRL NRW) Statusbewertung</div>
                    </div>
                    <button type="button" onclick="closeWrrlQualityModal()" aria-label="Schließen" title="Schließen" style="background: transparent; border: none; color: #ffffff; font-size: 1.25rem; cursor: pointer;">✕</button>
                </div>

                <div style="padding: 18px;">
                    ${!data.isReal ? `
                    <div style="background: #fffbeb; border: 1px solid #fef3c7; color: #92400e; padding: 8px; border-radius: 6px; margin-bottom: 14px; font-size: 0.65625rem;">
                    ...
                    ` : `
                    <div style="background: #f0fdf4; border: 1px solid #dcfce7; color: #166534; padding: 8px; border-radius: 6px; margin-bottom: 14px; font-size: 0.65625rem;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 14px;">
                        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 10px;">
                            <div style="font-size: 0.625rem; color: #64748b;">Ökologischer Zustand</div>
                            <div style="font-size: 0.8125rem; font-weight: 700; color: #0f172a; margin-top: 2px;">${escapeHtml(data.ecoStatus)}</div>
                        </div>
                        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 10px;">
                            <div style="font-size: 0.625rem; color: #64748b;">Chemischer Zustand</div>
                            <div style="font-size: 0.8125rem; font-weight: 700; color: #0f172a; margin-top: 2px;">${escapeHtml(data.chemStatus)}</div>
                        </div>
                    </div>

                    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 10px; margin-bottom: 14px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                            <span style="font-size: 0.65625rem; color: #64748b;">Durchgängigkeit für Fische:</span>
                            <span style="font-weight: 600; color: #0284c7;">${escapeHtml(data.fishPassability)}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-size: 0.65625rem; color: #64748b;">Entwicklungstrend:</span>
                            <span style="font-weight: 600; color: #16a34a;">${escapeHtml(data.trend)}</span>
                        </div>
                    </div>

                    <div style="display: flex; gap: 6px;">
                        <button type="button" class="btn btn-sm btn-outline-secondary" style="flex:1; font-size: 0.65625rem;" onclick="openWrrlQualityModal('Rur')">Rur</button>
                        <button type="button" class="btn btn-sm btn-outline-secondary" style="flex:1; font-size: 0.65625rem;" onclick="openWrrlQualityModal('Inde')">Inde</button>
                        <button type="button" class="btn btn-sm btn-outline-secondary" style="flex:1; font-size: 0.65625rem;" onclick="openWrrlQualityModal('Wurm')">Wurm</button>
                        <button type="button" class="btn btn-sm btn-outline-secondary" style="flex:1; font-size: 0.65625rem;" onclick="openWrrlQualityModal('Erft')">Erft</button>
                    </div>
                </div>
            </div>
        `;
