    ...
    async with async_playwright() as p:
        ...
        if frame:
            ...
            if await search_btn.count() > 0:
                ...
                excel_btn = frame.locator("input[value='Excel Export']")
                if await excel_btn.count() > 0:
                    print("Found 'Excel Export' button. Attempting download...")
                    ...
                    download_path = os.path.join(os.getcwd(), "elwas_export.xlsx")
                    await download.save_as(download_path)
                    print(f"Excel file successfully downloaded and saved to: {download_path}")
                    print(f"Downloaded file size: {os.path.getsize(download_path)} bytes")
                else:
                    print("Excel Export button not found!")
            else:
